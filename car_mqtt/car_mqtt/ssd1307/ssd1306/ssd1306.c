...
//上下
// Fill the upper half of the screen with the given color
void ssd1306_FillUpperHalf(SSD1306_COLOR color) {
    // 计算上半部分缓冲区的大小
    uint32_t upperHalfSize = sizeof(SSD1306_Buffer) / 2;
    uint32_t i;

    for(i = 0; i < upperHalfSize; i++) {
        SSD1306_Buffer[i] = (color == Black) ? 0x00 : 0xFF;
    }
}

// Fill the lower half of the screen with the given color
void ssd1306_FillLowerHalf(SSD1306_COLOR color) {
    // 计算上半部分缓冲区的大小
    uint32_t upperHalfSize = sizeof(SSD1306_Buffer) / 2;
    uint32_t i;

    for(i = upperHalfSize; i < sizeof(SSD1306_Buffer); i++) {
        SSD1306_Buffer[i] = (color == Black) ? 0x00 : 0xFF;
    }
}
